...
const Activities = ({ activityData }) => {
  const extractActivity = () => {
    const message = activityData.map((activity) => {
      ...
      let time = new Date(activity.created_at).toDateString().split(' ').slice(1).join(' ');
  ...
  const buildActivityList = () => {
    ...
    if (messages.length !== 0) {
        ...
        <li key={message.repoName}>
          <ActivitiesItem>
            <ActivityDiv>
              ...
              <a href={`https://github.com/${message.repoName}`}>{message.repoName}</a>
            </ActivityDiv>
      ...
      return (
        <li>
          <ActivitiesItem>
            <FlexContainer>No recent activities found :(</FlexContainer>
          </ActivitiesItem>
        </li>
      );
    }
